...
def test_user_cannot_access_others_notes(user, api_client):
    ...
    response =client_a.post(
        NOTES_URL, 
        {'title': 'User A Note', 
        'content': 'This is a note for user A.'},
        format='json')
    
    assert response.status_code == 201
    note_id = response.data['id']
    
    user_b = User.objects.create_user(username='testuser2', password='testpass2')
...
def test_note_update(user, api_client):
    ...
    response = client.put(f'{NOTES_URL}{note_id}/', {
        'title': 'Updated Title', 
...
def test_user_cannot_update_others_note(user, api_client):
    ...
    response = client_b.put(f'{NOTES_URL}{note_id}/', {
        'title': 'Hacked Title', 
